...
//if homeWorkDone is null,true or any other value, the else blocl will run
...
/* STEP 2a: Add an event listener for the 'click' event on the 'Set Weather' button 
that invokes the setWeather() function below */
weatherButton.addEventListener('click',setWeather);

function setWeather() {
	let choice = select.value;
	let temperature = temp.value;
	/* STEP 2b: Craft an IF/ELSEIF/ELSE that changes the src attribute of the icon 
	element to the appropriate .svg file in the images folder */
	/*if(choice === 'sunny'){
	...
	/* STEP 3: Add a nested IF/ELSE statement inside the 'sunny' condition above that
	 tests for temperature, and if it is equal to or greater than 15, turn the page background orange, otherwise turn it lightblue */
	
	/* STEP 4: Logical operators - and, or, not (delete multi-line comment delimiters 
	below) */
...
switch(speed){
	// STEP 5b: Build out four cases, followed by a default
		...
		default:
			windComment.textContent = 'Ascalm as it gets';
